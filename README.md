...
Let the AI call tools and external services during a conversation. Connect any MCP (Model Context Protocol) server — web search, code execution, file access, and more. Tools appear inline with approval controls, so you stay in charge of what runs on your machine.
...
Track token usage in real time with a visual progress bar. The context window size is dynamically calculated based on each model's limits. When context grows too large, GetChat can compress it automatically — or you can trigger compression manually.
...
OpenAI, DeepSeek, Ollama, OpenRouter, Groq, or anything that speaks the OpenAI protocol. Add as many providers as you like, set a default model for quick chats, or pick different models per branch. Your local Ollama models work out of the box.
...
| Layer | Technology |
|-------|-----------|
| Frontend | React 19, TypeScript (strict), Zustand, Tailwind CSS |
