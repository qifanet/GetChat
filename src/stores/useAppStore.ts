  ...
  devtools(
    subscribeWithSelector(
      immer((set, get) => ({
          ...
          try {
            ...
            set(
              (s) => {
                ...
                s.helperModelId = bootstrapResult.helperModelId;
                s.systemPrompt = bootstrapResult.systemPrompt;
        ...
        loadSettings: async () => {
          const [providerDtos, defaultModelId, helperModelId, systemPrompt] = await Promise.all([
            tauriCmd.listProviders(),
            tauriCmd.getDefaultModel(),
            tauriCmd.getHelperModel(),
            tauriCmd.getSystemPrompt(),
          ]);
          const { providers, providerModels, providerOrder } = normalizeProviders(
            providerDtos
          );
        ...
        setHelperModel: async (modelId) => {
          await tauriCmd.setHelperModel(modelId);
          set(
            (s) => {
              s.helperModelId = modelId;
            },
            undefined,
            "settings/helperModelChanged"
          );
        },

        setSystemPrompt: async (prompt) => {
          const savedPrompt = await tauriCmd.setSystemPrompt(prompt);
          set(
            (s) => {
              s.systemPrompt = savedPrompt;
            },
            undefined,
            "settings/systemPromptChanged"
          );
          return savedPrompt;
        },
        ...
        openConversation: async (conversationId) => {
          ...
          try {
            ...
            // Sync workspace path to backend tool_executor
            if (snapshot.summary.workspacePath) {
              try {
                await tauriCmd.setConversationWorkspace(
                  conversationId,
                  snapshot.summary.workspacePath
                );
              } catch {
                // Non-critical: tool_executor sync failure shouldn't block conversation loading
              }
            }
          } catch (err: unknown) {
            const message =
            ...
            if (result?.title) {
              ...
              if (summary) {
                set(
                  ...
                  undefined,
                  "conversation/titleAutoGenerated"
                );
              }
            } else if (result?.skipReason) {
              console.warn(`[title] skipped for conv=${conversationId}: ${result.skipReason}`);
            }
          } catch (err) {
        ...
        deleteAssistantVariantMessage: async (messageId) => {
          await tauriCmd.deleteAssistantVariantMessage(messageId);
          set(
            (s) => {
              if (!s.activeSnapshot) return;
          ...
          set(
            (s) => {
              s.workspace.forkIntent = {
                ...intent,
                editMode: intent.editMode ?? "NEW_BRANCH",
              };
        ...
        setHistoryEditMode: (mode) => {
          set(
            (s) => {
              if (
                s.workspace.forkIntent?.sourceType !== "HISTORY_USER_EDIT"
              ) {
                return;
              }
              s.workspace.forkIntent.editMode = mode;
            },
            undefined,
            "workspace/historyEditModeChanged"
          );
        },
