...
export function findForkPoint(
  ...
  // Collect all ancestor IDs for the left path
  ...
  // Walk the right path and find the first ancestor NOT in left path
  cursor = rightHeadId;
  let lastCommon: MessageId | null = null;

  while (cursor) {
    if (!leftAncestors.has(cursor)) {
      // This node diverges — the fork point is its parent
  ...
  // One path is a prefix of the other — they share the entire shorter path
  return lastCommon;
}
