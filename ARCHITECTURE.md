...
│  ├─ tools/                   ToolRegistry + 每工具一文件 + PolicyEngine + 并行执行器
│  │   ├─ registry.rs  policy.rs  executor.rs（含并行调度）
│  │   └─ builtin/{file,terminal,todo,web_search,skill,branch_fork,calculator}.rs
